 ...
 * and the header (conversation). A slow, smooth glide on a decelerating
 ...
 * "See more" modal (and shrinks back). Same decelerating curve as the mascot — a
...
/**
 * "See more" modal entrance: a clean, UNIFORM scale-up (0.96→1) + fade, centered. A
 ...
 * smooth — the "gentle grow" that presentations/alerts actually use. Decelerating, no bounce.
 */
...
/**
 * The active-segment indicator sliding between options of a segmented control (Text/Gen UI,
 * Chat/Read, Local/Groq). Register 2 (structural morph) at a small scale — the fill TRAVELS to
 * the chosen segment ("todo fluye a un lugar", DESIGN §9) instead of popping. Same decelerating
 * curve as the big morphs, just shorter so a control tap stays crisp. No bounce.
 */
export const SEGMENT_SLIDE: Transition = { duration: 0.3, ease: [0.32, 0.72, 0, 1] };

/**
 * Shared `layoutId` for the description surface. The small popover and the "See more"
 * modal both tag their card with this id, so Framer treats them as ONE object that
 * grows/shrinks between the two — the shared-element morph. Only one element
